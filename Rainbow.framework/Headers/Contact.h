...
/**
 *  When we sent an invitation to join a company to this user
 *  through REST api it is linked here.
 *  @see CompanyInvitation
 */
...
/**
 *  Whether the user is closed his account or not
 */
...
/**
 * Wheter the contact came from an external server (for exemple Active directory)
 */
@property (nonatomic, readonly) BOOL isExternalContact;

/**
 * Wheter the contact came from a company directory
 */
@property (nonatomic, readonly) BOOL isCompanyDirectoryContact;
...
/**
 * The user's associated Rainbow TV device
 * @see RainbowTV
 */
@property (nonatomic, readonly) RainbowTV *rainbowTV;

// -- Function helpers

/**
 * Returns the phone number that match the `PhoneNumberType`
 * @param type the expected phone number type
 * @return PhoneNumber
 */
-(PhoneNumber *) getPhoneNumberOfType:(PhoneNumberType) type;

/**
 * Returns YES if a phone number match the `PhoneNumberType`
 * @param type the expected phone number type
 * @return BOOL
 */
...
/**
 * Returns the phone number that match the `PhoneNumberType` and the `PhoneNumberDeviceType`
 * @param type the expected phone number type
 * @param deviceType the exected device type
 * @return PhoneNumber
 */
-(PhoneNumber *) getPhoneNumberOfType:(PhoneNumberType) type withDeviceType:(PhoneNumberDeviceType) deviceType;

/**
 * Returns YES if a phone number match the `PhoneNumberType` and the `PhoneNumberDeviceType`
 * @param type the expected phone number type
 * @param deviceType the exected device type
 * @return BOOL
 */
...
/**
 * Returns the user extension phone number (landline device)
 * @return PhoneNumber
 * @see PhoneNumberDeviceTypeLandline
 */
-(PhoneNumber *) getExtensionPhoneNumber;

/**
 * Returns the email address that match the `EmailAddressType`
 * @param type the expected email adress type
 * @return EmailAddress
 */
-(EmailAddress *) getEmailAdressOfType:(EmailAddressType) type;

/**
 * Returns YES if an email address match the `EmailAddressType`
 * @param type the expected email adress type
 * @return BOOL
 */
...
/**
 * Returns the phone number object reprensentation from a string
 * @param numberString the expected phone number string
 * @return PhoneNumber
 */
-(PhoneNumber*) getPhoneNumberFromString:(NSString*) numberString;

/**
 * Returns the phone number object reprensentation from a string and the `PhoneNumberType`
 * @param numberString the expected phone number string
 * @param PhoneNumberType the exected phone number type
 * @return PhoneNumber
 */
-(PhoneNumber*) getPhoneNumberFromString:(NSString*) numberString withType:(PhoneNumberType) type;

/**
 * Returns YES if the user is busy and in presentation
 */
-(BOOL) isBusyPresentation;
